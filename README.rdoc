...
=== 5. Add autoload_paths

    Add add some autoload_paths in application.rb:
...
* Grid actions
  ...
  - First, call the action method in grid configuration file, we want to add an 'print' button on the toolbar
  ...
  - Then, create a javascript file for the action, write the click handler for it:
...
* Grid behaviors
  ...
  - First, call the behavior method in grid configuration file, we want to add an 'show_total_price' behavior after grid data loaded.
  ...
  - Then, create a javascript file for this behavior, write the event and handler for it:
