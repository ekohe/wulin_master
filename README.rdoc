...
3. After you install wulin_master gem, you need run the generator:
  ...
  "grid_states" table is used for store the grid states for each user.
...
4. Assume generator a new grid called 'grid_test', run the generator:
   ...
   You can configre your gird in <tt>app/grids/grid_test_grid.rb</tt>:
   ...
   screen with "grid" method, and config the 'title' and the 'path'.
