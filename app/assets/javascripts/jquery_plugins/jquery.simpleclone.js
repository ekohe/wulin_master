...
$.fn.simple_clone = function(option){
  ...
  e.each(function(el) {
    $(this).find("input, select").each(function(){
      var old_id = $(this).attr("id");
      $(this).attr("id", old_id+'_'+index);
      if(option.nested == true) {
        var old_name = $(this).attr("name");
        var reg = /\[\w*\]$/;
        var match = reg.exec(old_name);
        var new_name = old_name.substr(0, match.index) + "[" + index + "]" + match[0];
        $(this).attr("name", new_name);
      }
    });
    index += 1;
  ...
  function setupEvents() {
    // ---------------------------- '+' button event ---------------------------------------------------------
    $("span.simple_plus").off('click').on("click", function(){
    ...
    // ---------------------------- '-' button event ----------------------------------------------------------
    $("span.simple_minus").off('click').on("click", function(){
      var outer_wrapper = $(this).closest(".outer_simple_wrapper");
      var wrapper_count = outer_wrapper.find(".simple_wrapper").length;
