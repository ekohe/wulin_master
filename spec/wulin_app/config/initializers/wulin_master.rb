# frozen_string_literal: true

WulinMaster.configure do |config|
  # The title of the page, default value is "Undefined App"
  config.app_title = "Welcome to Wulin World"

  # The height of the application title
  # config.app_title_height = '40px'

  # For the create popup form, when click "Create and Continue" button reset the form or not
  # config.always_reset_form = false

  # The default year
  # config.default_year = 2017

  # The default month
  # config.default_month = 2

  # The default color theme for the whole app
  # config.color_theme = 'blue'

  # The default master detail color
  # config.master_detail_color_theme = 'teal'

  # The default button mode for the whole app
  # config.button_mode = 'split'

  # The default nav sidebar partial path for the whole app
  # config.nav_sidebar_partial_path = ''
end
