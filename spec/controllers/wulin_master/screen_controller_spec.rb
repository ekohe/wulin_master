...
describe CountriesController, type: :controller do
  ...
  describe "Includes WulinMaster::Actions" do
    before :each do
      ...
      @grid = double
      # @grid.model(Country)
      allow(@grid).to receive(:model).and_return(Country)
    ...
    describe "get 'index'" do
      ...
      describe "format json" do
        before :each do
          allow(Country).to receive(:count).and_return(100)
          allow(Country).to receive(:all).and_return([])
          @mock_model = double("Country")
          controller = double
          allow(controller).to receive(:query).and_return(@mock_model)
        end
        ...
        it "should return a json object after calling render_json" do
          allow(@grid).to receive(:arraify).and_return(%w[China USA])
          ...
          expect(result).to include("[\"China\",\"USA\"]")
        end
      end
    end
