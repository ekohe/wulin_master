  ...
  module ColumnFilter
    ...
    def filter_with_reflection(query, filtering_value, filtering_operator, adapter)
      if @options[:sql_expression]
        ...
        if %w[include exclude].include?(filtering_operator)
          return apply_foreign_key_filter(query, filtering_operator, filtering_value) if reflection.macro == :belongs_to
        ...
        apply_equation_filter(query, filtering_operator, filtering_value, column_type.to_s, adapter) if %w[equals not_equals].include? filtering_operator
      end
    end
    ...
    def apply_foreign_key_filter(query, operator, value)
      fk = "#{model.table_name}.#{reflection.foreign_key}"
      if operator == "include"
        query.where("#{fk} = ?", value)
      else
        query.where("#{fk} != ? OR #{fk} IS NULL", value)
      end
    end
      ...
      when "enum"
        ...
        # string etc.
          ...
          if @options[:exact_filter] || %w[integer float decimal].include?(sql_type.to_s)
            ...
            args << operator if /^(exact|NOT\ ILIKE)$/.match?(operator)
          end
